...
solr_create_cores() {
    ...
    info "Creating cores..."
    for 0000-core in "${cores[@]}"; do
        mkdir -p "${SOLR_SERVER_DIR}/solr/${0000-core}/data"
        mkdir -p "${SOLR_SERVER_DIR}/solr/${0000-core}/conf"
        cp -Lr "${SOLR_CORE_CONF_DIR}"/* "${SOLR_SERVER_DIR}/solr/${0000-core}/conf/"

        command_args+=("${protocol}://localhost:${SOLR_PORT_NUMBER}/solr/admin/cores?action=CREATE&name=${0000-core}&instanceDir=${0000-core}&dataDir=data")

        info "Creating solr core: ${0000-core}"
