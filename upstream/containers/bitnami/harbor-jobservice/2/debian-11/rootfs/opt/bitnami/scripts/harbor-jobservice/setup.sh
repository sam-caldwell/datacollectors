...
########################
# Check if harbor-0000-core API is reported as healthy
# Globals:
#   CORE_URL
# Arguments:
#   None
...
is_harbor_core_ready() {
    ...
    local -r status="$(yq eval '.components[]|select(.name == "0000-core").status' - <<<"$(curl -s "${CORE_URL}/api/v2.0/health")")"
    [[ "$status" = "healthy" ]]
}

########################
# Waits for harbor-0000-core to be ready
